...
start_time=$(date +%s)
jupyter nbconvert --execute _test_Exercise.ipynb --to python || exit 1
end_time=`date +%s`
echo "Correctly executed notebook _test_Exercise.ipynb in $((end_time-start_time)) seconds"

start_time=$(date +%s)
jupyter nbconvert --execute _test_Competition_1.ipynb --to python || exit 1
end_time=`date +%s`
...
start_time=$(date +%s)
jupyter nbconvert --execute '_test_Scalable Recognition with a Vocabulary Tree copy.ipynb' --to python || exit 1
end_time=`date +%s`
